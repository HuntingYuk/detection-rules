...
tok=b(b'{"alg":"HS256","typ":"JWT"}')+"."+b(b'{"sub":"alice"}')+".c2lnbmF0dXJl"
req=b"GET /api/me HTTP/1.1\r\nHost: app.example\r\nAuthorization: Bearer "+tok.encode()+b"\r\nConnection: close\r\n\r\n"
from scapy.all import wrpcap, Ether, IP, TCP, Raw
S="10.0.0.82";D="10.0.0.80"
C=lambda f,s,a,p=b"":Ether()/IP(src=S,dst=D)/TCP(sport=52200,dport=80,flags=f,seq=s,ack=a)/(Raw(p) if p else b"")
V=lambda f,s,a:Ether()/IP(src=D,dst=S)/TCP(sport=80,dport=52200,flags=f,seq=s,ack=a)
