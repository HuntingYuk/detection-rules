...
# Rule has detection_filter count 3 -> fires only when matches EXCEED 3.
# Send count+1 = 4 matching requests from one src.
uri=b"/search?q=<iframe%20srcdoc=%22<script>alert(document.domain)</script>%22></iframe>"
S="10.0.0.59";pkts=[];t=1_700_000_000.0
for i in range(4):
    s=44610+i
