...
C=lambda f,s,a,p=b"":Ether()/IP(src=S,dst=D)/TCP(sport=44593,dport=80,flags=f,seq=s,ack=a)/(Raw(p) if p else b"")
V=lambda f,s,a:Ether()/IP(src=D,dst=S)/TCP(sport=80,dport=44593,flags=f,seq=s,ack=a)
